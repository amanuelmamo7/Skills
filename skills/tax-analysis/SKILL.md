---
name: tax-analysis
description: Analyze US federal income tax questions through the four-stage individual framework — gross income and exclusions, deductions (business, capital recovery, mixed-purpose), timing and accounting methods, and attribution (who is the taxpayer) — plus a state-and-local overlay for nexus and constitutional constraints and a Subchapter K entity overlay for partnerships and LLCs (classification and check-the-box, formation, allocations and substantial economic effect, liabilities and basis, distributions, sales of interests, retirement, S corporation contrast). Reads the actual Internal Revenue Code at runtime via a bundled section-to-page index, so every Code citation is quoted from the statute, never recalled. Use when the user asks any tax question — "is this income," "can I deduct this," "how is this gain taxed," "when is this recognized," "who pays tax on this," "can the state tax this," "how is my LLC or partnership taxed," "what happens when a partner contributes, is paid, or cashes out" — or wants a tax memo, issue-spot, or Code section explained. Differentiator: Code-first with runtime verification; analysis support for a tax professional, never tax advice or return preparation.
---
...
> Born from: a complete federal individual income tax course framework (outline, case briefs, and SALT constitutional materials) authored in study, extended with a Subchapter K module distilled as methodology from partnership-taxation study texts (doctrine structure only — no text reproduced), plus the verification-tier discipline of the attorney-workflow family applied to the one domain where an invented citation is most tempting: statutory text everyone half-remembers. Built with `skill-distiller`.
...
**Entity overlay — partnerships and LLCs.** When the taxpayer is (or is becoming) a partnership, an LLC, or a partner/member: run `references/partnership-tax-framework.md` before the four stages, because Subchapter K decides *whose* item it is and *what character* it carries before the individual framework can price it. The map runs the lifecycle — classification and check-the-box (Reg. §301.7701; §7704), formation (§§721–723 with the §752 encumbered-property trap), operations and the loss-limitation gauntlet (§704(d) → §465 → §469 → §461(l)), allocations (§704(b) substantial economic effect; §704(c) built-in gain; §704(e)), liabilities (§752 economic risk of loss; nonrecourse tiers), the service partner (§§707, 83; profits interests; §1061 carried interests; SE tax), distributions (§§731–737, 751(b)), sales of interests (§§741, 751(a), 743(b)/754), retirement and death (§736), and an S corporation contrast for choice-of-entity questions. Its three safeguards — allocations must track economics, precontribution gain stays home, ordinary income can't be laundered through the entity — are where nearly every hard entity question lives.
...
- `references/partnership-tax-framework.md` — the Subchapter K entity map: classification through liquidation, plus the S corporation contrast.
...
The Code snapshot ages: after the next major tax act, the page index needs regeneration against a new PDF (the indexing script pattern is one regex over pdftotext output — rebuild rather than patch). The entity overlay now covers Subchapter K and sketches Subchapter S, but Subchapter C corporate tax (distributions, redemptions, reorganizations) remains out of scope — if questions trend there, that module is the next build. The partnership map was distilled from pre-2020 study texts, so its TCJA-era figures and any provision with a sunset (§199A, §461(l)) are standing verification targets, and its reliance on proposed or expired regulations (LLC-member SE tax, §469 limited-partner status, §751(b)) must be re-checked whenever finalized rules appear. And if a quoted "current" threshold is ever shown stale in use, the currency-flag discipline failed structurally — tighten the Rev. Proc. check from flag to mandatory lookup.
