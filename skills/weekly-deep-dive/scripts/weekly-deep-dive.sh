#!/bin/bash
# weekly-deep-dive.sh — Sefer's Sunday weekly deep-dive
#
...
# reaped the subshell before it polled/delivered, so weekly NEVER delivered to
# Telegram or the dashboard (the brief files exist only because Sefer writes
# them via the gateway). Now we poll + deliver inline so the script stays alive.
#
...
ISOWEEK=$(date +%G-W%V)
LOG="$WORKSPACE/logs/cron-$(date +%Y-%m-%d)-weekly-deep-dive.log"
mkdir -p "$(dirname "$LOG")"

ts() { date -Iseconds; }
log() { echo "$(ts) $*" >> "$LOG"; }

log "===== weekly-deep-dive.sh start ($ISOWEEK) ====="
...
PROMPT=$(cat "$WORKSPACE/prompts/weekly-deep-dive.txt")
...
BRIEF="$WORKSPACE/memory/${ISOWEEK}-deep-dive.md"
TELEGRAM="$WORKSPACE/memory/${ISOWEEK}-deep-dive-telegram.txt"
DIGEST="$WORKSPACE/memory/${ISOWEEK}-deep-dive.json"
...
if [ ! -f "$BRIEF" ]; then
  log "[2/4] ERROR: deep-dive never appeared after ~40min"
  ...
  curl -s --max-time 30 -X POST "https://api.telegram.org/bot${BOT_TOKEN}/sendMessage" \
    -d "chat_id=${CHAT_ID}" \
    --data-urlencode "text=Sefer weekly-deep-dive: brief not delivered — timed out. Gateway said: ${GWERR:-no error logged}. Log: $LOG" \
...
if [ ! -f "$FINANCE_TOKEN_FILE" ]; then
  ...
  POST_BODY=$(jq -nc --arg cadence "weekly" --arg md "$(cat "$BRIEF")" --arg telegram "$TG_TXT" \
...
log "===== weekly-deep-dive.sh complete ====="
